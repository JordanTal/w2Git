...
if [[ !( "${#pass}" -ge 10 ) ]]
  then
    echo "$(tput setaf 1).. -_- Password is too short , password must contain at least 10 characters .. -_-"
...
if [[ !("$pass" == *[0-9]* )]]
  then
	echo "$(tput setaf 1)... -_- Password MUST contain at least 1 digit... -_- "
	validity=false
fi
# Characters existance (Upper & Lower) check
if [[ !(("$pass" == *[A-Z]*) && ("$pass" == *[a-z]*))]]
  then
	echo "$(tput setaf 1).... -_- Password MUST include both the small and capital case letters .... -_- "
	validity=false
fi
