...
{
     Write-Host "`n ! password MUST have at least 10 charachters `n "
     $pass_validity = $false
}
# Check that the password uses both lower and upper case charachters
if(!(($pass -cmatch '[a-z]') -and ($pass -cmatch '[A-Z]')))
{ 
    Write-Host "`n !! password must have at least ONE Lower case and ONE Upper case characters in it !  `n "
         $pass_validity = $false
}
# Check that password includes at least one digit 
if(!($pass -match '\d'))
{
     Write-Host "`n !!! password must contain At least one digit! `n "
          $pass_validity = $false
}
...
{
    Write-Host "`n`n`n)-: As seen above - password does not meet at least one of the requirements... `n"
    exit
}
